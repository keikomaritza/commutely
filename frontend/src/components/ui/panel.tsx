...
export function MapPanel({ title, children, side = "bottom", className, ...props }: HTMLAttributes<HTMLElement> & {
  ...
  return <aside className={cn("ui-slide-up absolute z-20 overflow-y-auto rounded-t-[var(--radius-xl)] border border-[var(--color-line)] bg-white/98 p-5 shadow-[var(--shadow-float)] backdrop-blur", placement[side], className)} {...props}>
