...
export function RoutePlanner({ onRouteChange }: RoutePlannerProps) {
  ...
  return (
    <section className="grid gap-4">
      ...
      {loading && (
        <p className="ui-fade-in flex items-center gap-2 text-sm text-[var(--color-muted)]">
          <span aria-hidden="true" className="ui-spin inline-block size-3 rounded-full border-2 border-[var(--color-line)] border-t-[var(--color-primary)]" />Menghitung rute...
        </p>
      )}
      ...
      {error && (
        <p
          role="alert"
          className="ui-slide-up rounded-[var(--radius-md)] border border-red-100 bg-red-50 p-3 text-sm text-red-700"
        >
          {error}
        </p>
      )}

      {route && <div className="ui-slide-up"><RouteResult route={route} /></div>}
